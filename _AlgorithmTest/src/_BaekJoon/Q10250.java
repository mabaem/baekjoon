 ...
 */
	...
	public static void main(String args[]) throws IOException {
		...
		for (int i=0; i<T; i++) {
			...
			int H = Integer.parseInt(st.nextToken());	//호텔 층수(높이)
			int W = Integer.parseInt(st.nextToken());	//호텔 호수(너비)
			int N = Integer.parseInt(st.nextToken());	//손님 순서
			
			sb.append(room_number(H, W, N)).append("\n");
		}
	...
	public static String room_number(int height, int width, int guest) {
		String result = "";
		
		if ((guest % height) == 0) {
			//방번호 = 층수*100 + 손님순서/층수
			result = Integer.toString((height*100) + (guest/height));
		}
		else {
			result = Integer.toString(((guest%height)*100) + ((guest/height)+1));
		}
